...
pub fn build_memtable_key(key: &[u8], mt: MsgType, msn: MSN) -> Vec<u8> {
    ...
    buf.resize(keysize + keysize.required_space(), 0);

    {
        ...
        writer
            .write_fixedint((mt as u64) | (msn << 8))
            .expect("write to slice failed");
...
pub fn cmp_memtable_key<'b>(ucmp: &dyn Cmp, a: MemtableKey, b: MemtableKey) -> Ordering {
